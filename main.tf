...
resource "aws_subnet" "custom_subnet_1a" {
  ...
  map_public_ip_on_launch = true  # assign a public IP address upon launch
...
resource "aws_security_group" "wordpress_sg" {
  depends_on = [aws_vpc.custom_vpc]  # a certain resource should wait for another resource to be created or modified before it is itself created or modified
